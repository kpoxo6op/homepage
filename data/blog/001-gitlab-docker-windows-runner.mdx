...
Second, we register the runner with Docker for Windows executor. Terraform at [gcp_windows_vm.tf](#review-gcp_windows_vmtf) substitutes `${runner_roken}` with the
...
Maybe it's possible to excute Docker and Runner in [separate](https://cloud.google.com/compute/docs/instances/startup-scripts/windows) Google startup scripts to make the code simpler. I could not make it working, either Docker or Runner parts would break.
