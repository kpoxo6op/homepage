...
- Thomas'es guide at [codingwiththomas.com](https://www.codingwiththomas.com/blog/windows-docker-container-in-gitlab-with-conan-packages). Thomas creates his own Dockerfile for Windows container (`--docker-image` parameter for runner executable). Watch out for `--tag-list` parameter as it no longer works. Tags need to be set at the server now. I set them in [gitlab.tf](#review-gitlabtf) im my guide.