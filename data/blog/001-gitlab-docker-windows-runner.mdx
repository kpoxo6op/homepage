...
- `--token`: Terraform at [gcp_windows_vm.tf](#review-gcp_windows_vmtf) will substitute `${runner_roken}` with the
  token from [gitlab.tf](#review-gitlabtf)
- Images are set to nanoserver version because we don't want to download over 10GB of data with the default images.
