...
Recently, I had the opportunity to provision a new GitLab Runner with Docker for Windows Executor. The Windows VM which hosted the runner needed to be configured with Terraform and PowerShell.

I discovered that the GitLab documentation was a bit inaccurate, outdated, or not detailed enough.

I also did not find guides how to provision a Runner in unattended mode.

For these reasons, I decided to publish this article. I hope makes other people's work easer.
...
The guide is focussed on automation rather than security, so trade off were made to keep it short:

- Terraform state and access keys are stored locally
- We use the single GCP project for everything
- Service account permissions are too loose
...
_Note_: Google provides a credit for all new accounts, so you  won't be charged. Just don't forget to clean up resources when you are done.
...
Create any Group and Project names during the sign up process (GitLab won't let you skip it). We will create another group with terraform later.
...
Create the `.env` file. Terraform will 
...
[gcp_windows_vm.tf](#review-gcp_windows_vmtf) substitutes `${runner_roken}` with the
...
Finally, install, start and verify the runner service.
