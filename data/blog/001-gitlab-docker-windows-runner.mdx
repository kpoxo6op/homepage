...
Also, the machine hosting the runner needed to be created and configured automatically. I found no examples of automatic setup on the internet.
...
The guide is focussed on automation rather than security, so trade off were made to keep it short. Terraform state is stored locally, we use the single GCP project for everyhing The same GCP account manages terraform state and infrastructure
