...
on the Splunk support forum explaining how to automatically resolve PagerDuty
alerts received from Splunk.

It took me some time to understand the post, so I decided to expand it into an
easy-to-follow guide to help others.

It's remarkable that it's been four years since the original post but there is
still no support for autoresolving Splunk alerts as per PagerDuty
[docs](https://www.pagerduty.com/docs/guides/splunk-integration-guide/)
...
Clone GitHub
[repository](https://github.com/kpoxo6op/splunk-pagerduty-autoresolve) with the
demo code.
...
Login to local Splunk with credentials from `.env` file (default is `admin/abcd1234`).
...
PagerDuty should start creating and resolving Incidents automatically.
...
resolving alerts which are no longer firing.
...
This guide fills the gap in the PagerDuty Splunk app by showing how to
auto-resolve alerts using Splunk KV Store and PagerDuty Service Orchestration.