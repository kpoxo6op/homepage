...
Run the main query with the KV store output on the side and to see the alert state tracking in action.

```sql
| inputlookup state_alert
```

![Runner Job Complete](/static/images/002-resolvable-splunk-alerts-pagerduty/splk-queries.png)

Trigger Action sends the alert event to PagerDuty with `event_action` in message `custom_details`.

`event_action` can be Trigger or Resolve whether we want to create or resolve an incident in PagerDuty.
...
PagerDuty Splunk App sets `event_action`. It is always `trigger` because the app [does not support](https://www.pagerduty.com/docs/guides/splunk-integration-guide/) resolving alerts which are no logner firing.

The Splunk Alert Search sets `custom_details.event_action`. It can be `Resolve` or `Trigger`.

### PagerDuty Service Orchestration

PagerDuty evaluates `event_action` and [creates](https://developer.pagerduty.com/docs/ZG9jOjExMDI5NTgx-send-an-alert-event#event-action-behavior) an incident on the Default Service. This is a standard out-of-the box behavior.

The interesting and most important part happens at PagerDuty Event Orchestration rule called `Autotoresolve Splunk Alerts`. PagerDuty resolves an incident if it finds `resolve` in `custom_details`.

The rule is defined by `pagerduty_event_orchestration_service` terragorm resource:
